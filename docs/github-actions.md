...
- На `push` в `master` автоматически повышает `appVersion`, создаёт commit и git tag `vX.Y.Z`.
...
3. Merge PR в `master` -> `Build Images` (bump версии, tag, публикация образов в GHCR, sync `master -> develop`).
