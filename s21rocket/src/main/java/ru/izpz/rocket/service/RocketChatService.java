...
public class RocketChatService {
  ...
  private final AtomicReference<CompletableFuture<RocketChatSendResponse>> qrInFlight =
      new AtomicReference<>();
  ...
  public void validateConfiguration() {
    ...
    if (!properties.getWebsocketUri().startsWith("ws://")
        && !properties.getWebsocketUri().startsWith("wss://")) {
    ...
    log.info(
        "Rocket.Chat service initialized successfully with URL: {}, bot: {}, QR timeout: {}s, message timeout: {}s",
        properties.getWebsocketUri(),
  ...
  public RocketChatSendResponse generateQrCode() {
    ...
    try {
      ...
      RocketChatSendResponse failure =
          new RocketChatSendResponse(false, "Failed to generate QR code: " + e.getMessage());
