...
public class ListKeyboardFactory {
  ...
  public <T> String createListText(List<T> items, String title, Function<T, String> itemFormatter) {
    ...
    for (int i = 0; i < items.size(); i++) {
      T item = items.get(i);
      int ordinal = i + 1;
      sb.append(ordinal).append(". ").append(itemFormatter.apply(item)).append("\n");
    }
  ...
  public String eventsListText(EventsSliceDto events) {
    return createListText(
        events.content(), "\uD83D\uDE38 События в кампусе \uD83D\uDE3D", EventDto::name);
  }

  public String friendsListText(FriendsSliceDto friends) {
    return createListText(
        friends.content(), "\uD83D\uDE38 Мои друзья \uD83D\uDE3D", this::formatFriend);
  }
  ...
  private String formatFriend(ru.izpz.dto.FriendDto friend) {
    ...
    if (Boolean.TRUE.equals(friend.getIsOnline())) {
      sb.append("\uD83D\uDFE2");
      sb.append(" ")
          .append(friend.getClusterName())
          .append("-")
          .append(friend.getRow())
          .append(friend.getNumber());
    } else {
      sb.append("\uD83D\uDCA4");
    }
