...
public enum MenuCommandEnum {
  ...
  private static final Map<String, MenuCommandEnum> COMMANDS_MAP =
      Arrays.stream(values()).collect(Collectors.toMap(MenuCommandEnum::getCommand, e -> e));
  ...
  public static List<String> getAllMenuCommands() {
    return Arrays.stream(values()).map(MenuCommandEnum::getCommand).collect(Collectors.toList());
  }
