...
public class BotConfig {
  ...
  public OkHttpTelegramClient telegramClient(
      OkHttpClient telegramOkHttpClient, BotProperties botProperties) {
    ...
    if (proxy != null && Boolean.TRUE.equals(proxy.enabled())) {
      if (!StringUtils.hasText(proxy.host()) || proxy.port() == null || proxy.port() <= 0) {
        throw new IllegalStateException(
            "BOT_PROXY_ENABLED=true, but BOT_PROXY_HOST/BOT_PROXY_PORT are invalid");
      }
      ...
      OkHttpClient okHttpClient =
          builder
              ...
              .build();
      log.info(
          "Telegram client proxy enabled: type={}, host={}, port={}",
          proxyType,
          proxy.host(),
          proxy.port());
      return okHttpClient;
  ...
  public TelegramBotsLongPollingApplication botsApplication(
      MetricsService metricsService, OkHttpClient telegramOkHttpClient) {
