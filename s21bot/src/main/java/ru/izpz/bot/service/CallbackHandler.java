...
public class CallbackHandler {
  ...
  public void handleCallbackMessage(
      Long chatId, String data, Integer messageId, String callbackId) {
    try {
      ...
      switch (payload.getCommand()) {
        case TelegramButtons.REGISTRATION_CODE ->
            updateMessageAndChangeStatusRegistration(
                chatId, messageId, "Введите логин на платформе");
        case "show_friend" -> {
        ...
        case "add_friend" ->
            applyAndRefreshKeyboard(
                chatId,
                messageId,
                callbackId,
                payload.getArgs().get(LOGIN),
                FriendRequest.Action.TOGGLE_FRIEND,
                "Статус «друг» переключён");
        case "favorite" ->
            applyAndRefreshKeyboard(
                chatId,
                messageId,
                callbackId,
                payload.getArgs().get(LOGIN),
                FriendRequest.Action.TOGGLE_FAVORITE,
        ...
        case "subscribe" ->
            applyAndRefreshKeyboard(
                ...
                callbackId,
                payload.getArgs().get(LOGIN),
                FriendRequest.Action.TOGGLE_SUBSCRIBE,
                "Статус подписки переключён");
        case "set_name" -> {
          setLastCommand(
              chatId, LastCommandType.SET_NAME, Map.of(LOGIN, payload.getArgs().get(LOGIN)));
        ...
        case "event" -> {
          ...
          messageSender.sendMessageWithoutWebPreview(
              chatId, EventMessageFormatter.format(eventDto), null);
        }
        case "events_page" -> {
  ...
  private void applyAndRefreshKeyboard(
    ...
    messageSender.execute(
        telegramKeyboardFactory.createAnswerCallbackQuery(callbackId, toastText, false));
  }
  ...
  public void showProfile(Long chatId, String login) {
    ...
    try {
      ...
      InlineKeyboardMarkup keyboard =
          telegramKeyboardFactory.getFriendInlineKeyboard(login, friend);
      ...
      messageSender.sendMessage(
          chatId, ParticipantMessageFormatter.format(showProfile, projects), keyboard);
    } catch (FeignException e) {
      ...
      messageSender.sendMessage(
          chatId, "Ошибка проверки логина: " + e.getError().getMessage(), null);
      messageSender.sendMessage(
          botProperties.admin(), "Ошибка проверки логина: " + e.getError(), null);
    }
  }
  ...
  public void updateMessageAndChangeStatusRegistration(
