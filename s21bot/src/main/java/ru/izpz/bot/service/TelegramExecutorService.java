...
public class TelegramExecutorService {
  ...
  public <T extends Serializable> Optional<T> execute(BotApiMethod<T> method) {
    ...
    try {
      ...
      metricsService.recordTelegramApiRequest(
          methodName, result.isPresent() ? "success" : ERROR_OUTCOME);
      return result;
      ...
      metricsService.recordTelegramApiRequest(methodName, ERROR_OUTCOME);
      log.error(
          "Неожиданная ошибка вызова Telegram API метода {}: {}", methodName, e.getMessage(), e);
      return Optional.empty();
    }
  }
