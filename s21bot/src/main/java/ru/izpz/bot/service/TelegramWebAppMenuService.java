...
public class TelegramWebAppMenuService {
  ...
  public void ensureMenuButton(Long chatId) {
    ...
    if (!isValidWebAppUrl(webAppUrl)) {
      log.warn(
          "Web App menu button skipped for chatId={} because bot.web-app-url is invalid or placeholder: {}",
          chatId,
          webAppUrl);
      return;
    }
    ...
    SetChatMenuButton method =
        SetChatMenuButton.builder().chatId(chatId.toString()).menuButton(menuButton).build();
