...
class CallbackHandlerTest {
  ...
  @Mock private TelegramKeyboardFactory telegramKeyboardFactory;

  @Mock private CallbackPayloadSerializer callbackPayloadSerializer;
  ...
  void handleCallbackMessage_invalidPayload_sendsErrorMessage() {
    when(callbackPayloadSerializer.deserialize("bad"))
        .thenThrow(new InvalidCallbackPayloadException("x", null));
  ...
  void handleCallbackMessage_unexpectedException_sendsGenericErrorMessage() {
    ...
    doThrow(new RuntimeException("boom"))
        .when(metricsService)
        .recordButtonPress("unknown", ButtonMetricType.INLINE);
  ...
  void handleCallbackMessage_friendsPage_callsShowFriends() {
    when(callbackPayloadSerializer.deserialize("data"))
        .thenReturn(new CallbackPayload("friends_page", Map.of("page", "2")));
    ...
    when(telegramKeyboardFactory.friendsListKeyboard(eq(slice), anyInt(), eq(2)))
  ...
  void handleCallbackMessage_eventsPage_callsShowEvents() {
    when(callbackPayloadSerializer.deserialize("data"))
        .thenReturn(new CallbackPayload("events_page", Map.of("page", "1")));
    ...
    when(telegramKeyboardFactory.eventsListKeyboard(eq(slice), anyInt(), eq(1)))
        .thenReturn(mock(InlineKeyboardMarkup.class));
  ...
  void handleCallbackMessage_showFriend_callsShowProfile() {
    when(callbackPayloadSerializer.deserialize("data"))
        .thenReturn(new CallbackPayload("show_friend", Map.of("login", "abc")));
    ...
    when(profileService.applyFriend(chatId, "abc", FriendRequest.Action.NONE, null))
        .thenReturn(friend);
    ...
    when(profileService.getProjects("abc"))
        .thenReturn(
            List.of(
                new ProjectsDto(
                    "g1", "Libft", null, null, null, null, null, null, null, null, null),
                new ProjectsDto(
                    "g2", "C Piscine", null, null, null, null, null, null, null, null, null)));
    ...
    verify(messageSender)
        .sendMessage(
            eq(chatId),
            argThat(text -> text.startsWith("✅ abc") && text.contains("📁Libft, C Piscine")),
            eq(kb));
  }
  ...
  void handleCallbackMessage_registration_updatesMessageAndSetsStatusRegistration() {
    when(callbackPayloadSerializer.deserialize("data"))
        .thenReturn(
            new CallbackPayload(ru.izpz.bot.keyboard.TelegramButtons.REGISTRATION_CODE, null));
  ...
  void handleCallbackMessage_addFriend_appliesFriendAndRefreshesKeyboardAndToasts() {
    ...
    when(profileService.applyFriend(chatId, "abc", FriendRequest.Action.TOGGLE_FRIEND, null))
        .thenReturn(friend);
    ...
    when(telegramKeyboardFactory.createAnswerCallbackQuery("cb", "Статус «друг» переключён", false))
        .thenReturn(answer);
  ...
  void handleCallbackMessage_favorite_appliesFriendToggleFavorite() {
    ...
    FriendDto friend = FriendDto.builder().login("abc").isFriend(true).build();
    when(profileService.applyFriend(chatId, "abc", FriendRequest.Action.TOGGLE_FAVORITE, null))
        .thenReturn(friend);
    when(telegramKeyboardFactory.getFriendInlineKeyboard("abc", friend))
        .thenReturn(mock(InlineKeyboardMarkup.class));
    when(telegramKeyboardFactory.editFriendInlineKeyboard(
            ArgumentMatchers.any(), eq(chatId), eq(7)))
        .thenReturn(mock(EditMessageReplyMarkup.class));
  ...
  void handleCallbackMessage_subscribe_appliesFriendToggleSubscribe() {
    ...
    FriendDto friend = FriendDto.builder().login("abc").isFriend(true).build();
    when(profileService.applyFriend(chatId, "abc", FriendRequest.Action.TOGGLE_SUBSCRIBE, null))
        .thenReturn(friend);
    when(telegramKeyboardFactory.getFriendInlineKeyboard("abc", friend))
        .thenReturn(mock(InlineKeyboardMarkup.class));
    when(telegramKeyboardFactory.editFriendInlineKeyboard(
  ...
  void showProfile_success_sendsProfileWithKeyboard() {
    ...
    when(profileService.getProjects("abc"))
        .thenReturn(
            List.of(
                new ProjectsDto(
                    "g1", "Exam_Rank_02", null, null, null, null, null, null, null, null, null)));
    ...
    verify(messageSender)
        .sendMessage(
            eq(chatId),
            argThat(text -> text.startsWith("✅ abc") && text.contains("📁Exam_Rank_02")),
            eq(kb));
  }
  ...
  void handleCallbackMessage_event_sendsFormattedEventCard() {
    ...
    verify(messageSender)
        .sendMessageWithoutWebPreview(
            eq(chatId),
            argThat(
                text -> {
                  String value = (String) text;
                  return value.startsWith("🗓️ День открытых дверей")
                      && value.contains("\n💬 Большая встреча\n")
                      && value.contains("📍 Москва, Кампус 21")
                      && value.contains("🙌️ 20.04.2026 10:00 - 20.04.2026 12:00")
                      && value.contains("👥 85 / 120")
                      && value.contains("👨‍💼 School 21, Community Team");
                }),
            isNull());
  }
  ...
  void showProfile_invalidLogin_sendsValidationMessage() {
    ...
    verify(messageSender)
        .sendMessage(
            chatId,
            "Логин должен быть от 3 до 30 символов и состоять только из латинских букв",
            null);
  ...
  void showProfile_whenProjectsUnavailable_sendsProfileWithoutProjectsLine() {
    ...
    verify(messageSender)
        .sendMessage(
            eq(chatId), argThat(text -> text.startsWith("✅ abc") && !text.contains("📁")), eq(kb));
  }
  ...
  void updateMessageAndChangeStatusRegistration_whenFeignException_doesNotThrow() {
    doThrow(createFeignException(500, "boom"))
        .when(profileService)
        .updateProfileStatus(chatId, ProfileStatus.REGISTRATION);
