...
class RegistrationFlowTest {
  ...
  @InjectMocks private RegistrationFlow registrationFlow;
  ...
  void startOnboarding_sendsRegistrationButton() {
    ...
    when(telegramButtons.getRegistrationButton())
        .thenReturn(java.util.Map.of("Регистрация", "data"));
  ...
  void startValidation_wrongCode_sendsRemoveKeyboard() {
    ...
    verify(profileService, never())
        .updateProfileStatus(ArgumentMatchers.anyLong(), ArgumentMatchers.any());
  ...
  void startRegistration_sendVerificationCodeSuccess_updatesStatusValidationAndNotifiesAdmin() {
    ...
    verify(messageSender)
        .sendMessage(
            chatId, "В рокет чат был отправлен код для подтверждения для логина abc", null);
  ...
  void startRegistration_whenCheckAndSetLoginReturnsDifferentLogin_doesNotStartValidation() {
    ...
    verify(messageSender, never())
        .sendMessage(eq(999L), ArgumentMatchers.anyString(), ArgumentMatchers.any());
  }
  ...
  void startRegistration_sendVerificationCodeThrowsRocketChatSendException_notifiesUserAndAdmin() {
    ...
    when(profileService.sendVerificationCode("abc"))
        .thenThrow(new RocketChatSendException(new RocketChatSendResponse(false, "fail")));
