...
public class CampusScheduler {
  ...
  public void parseMskKznNsk() {
    ...
    try {
      ...
      schedulerMetricsService.stopPhaseTimer(
          SCHEDULER_NAME, PHASE_CAMPUS_SNAPSHOT_REFRESH, clustersSample);
    }
    if (!clustersOutcome.completed()) {
      if (clustersOutcome.hasSuccessfulCampuses()) {
        ...
        log.warn(
            "Цикл парсинга завершен частично: часть кампусов обновлена, часть не успела до таймаута");
  ...
  private PhaseOutcome processClusters(List<String> campuses, StopWatch stopWatch) {
    ...
    try {
      ...
      PhaseOutcome baseOutcome =
          awaitPhaseResults(PHASE_CAMPUS_SNAPSHOT_REFRESH, clustersTimeout, futures);
      CampusRunSummary summary = summarizeCampuses(campuses, futures);
      return new PhaseOutcome(
          baseOutcome.completed(),
          baseOutcome.hasErrors(),
          baseOutcome.hasSuccessfulCampuses(),
          summary);
    } finally {
      stopWatch.stop();
    }
  ...
  private List<CompletableFuture<TaskResult>> submitClusterTasks(
      List<String> campuses, Duration campusTimeout) {
    return campuses.stream()
        .map(
            id ->
                CompletableFuture.supplyAsync(
                        () -> processSingleCampus(id), campusSchedulerExecutor)
                    .completeOnTimeout(
                        TaskResult.timeoutResult(), campusTimeout.toMillis(), TimeUnit.MILLISECONDS)
                    .thenApply(
                        result -> {
  ...
  private TaskResult processSingleCampusParticipants(String campusId) {
    try {
      ...
      schedulerMetricsService.recordExternalApiSuccess(
          SCHEDULER_NAME, CAMPUS_API_CLIENT, OPERATION_GET_CLUSTERS);
      ...
      schedulerMetricsService.recordExternalApiError(
          SCHEDULER_NAME, CAMPUS_API_CLIENT, OPERATION_GET_CLUSTERS, e);
      log.error(
          "Ошибка получения кластеров для кампуса {} ({})",
      ...
      schedulerMetricsService.recordExternalApiError(
          SCHEDULER_NAME, CAMPUS_API_CLIENT, OPERATION_GET_CLUSTERS, e);
      log.error(
          "Непредвиденная ошибка получения кластеров для кампуса {} ({})",
          campusCatalog.campusName(campusId),
          campusId,
          e);
  ...
  private TaskResult processSingleCampusParticipants(String campusId, List<ClusterV1DTO> clusters) {
    ...
    try {
      ...
      log.error(
          "Непредвиденная ошибка сохранения снапшота кампуса {} ({})",
          campusCatalog.campusName(campusId),
          campusId,
          e);
      return TaskResult.errorResult(SchedulerErrorReason.EXECUTION_EXCEPTION);
  ...
  private ClusterTaskOutcome processSingleCluster(ClusterV1DTO c) {
    ...
    try {
      ...
      schedulerMetricsService.recordExternalApiSuccess(
          SCHEDULER_NAME, CAMPUS_API_CLIENT, OPERATION_GET_PARTICIPANTS);
      ...
      schedulerMetricsService.recordExternalApiError(
          SCHEDULER_NAME, CAMPUS_API_CLIENT, OPERATION_GET_PARTICIPANTS, e);
      log.error("Ошибка получения участников для кластера {}", cid, e);
      return new ClusterTaskOutcome(
          TaskResult.errorResult(SchedulerErrorReason.API_EXCEPTION), List.of());
    } catch (Exception e) {
      schedulerMetricsService.recordExternalApiError(
          SCHEDULER_NAME, CAMPUS_API_CLIENT, OPERATION_GET_PARTICIPANTS, e);
      log.error("Непредвиденная ошибка получения участников для кластера {}", cid, e);
      return new ClusterTaskOutcome(
          TaskResult.errorResult(SchedulerErrorReason.EXECUTION_EXCEPTION), List.of());
    }
  }

  private PhaseOutcome awaitPhaseResults(
      String phase, Duration timeout, List<CompletableFuture<TaskResult>> futures) {
    ...
    try {
      ...
      cancelPendingFutures(futures);
      schedulerMetricsService.recordPhaseIssue(
          SCHEDULER_NAME, phase, SchedulerErrorReason.INTERRUPTED);
      log.error("Прервано ожидание результатов фазы {}", phase, e);
      return buildPhaseOutcome(false, futures, phase);
    } catch (Exception e) {
      schedulerMetricsService.recordPhaseIssue(
          SCHEDULER_NAME, phase, SchedulerErrorReason.EXECUTION_EXCEPTION);
  ...
  private PhaseOutcome buildPhaseOutcome(
      boolean completed, List<CompletableFuture<TaskResult>> futures, String phase) {
  ...
  private CampusRunSummary summarizeCampuses(
      List<String> campuses, List<CompletableFuture<TaskResult>> futures) {
    ...
    return new CampusRunSummary(
        total, successful, failed, timedOut, updatedCampuses, failedCampuses);
  }
  ...
  record TaskResult(
      boolean success, SchedulerPhaseRequestStatus status, SchedulerErrorReason errorType) {
    static TaskResult successResult() {
      return new TaskResult(true, SchedulerPhaseRequestStatus.SUCCESS, SchedulerErrorReason.NONE);
    }

    static TaskResult timeoutResult() {
      return new TaskResult(
          false, SchedulerPhaseRequestStatus.FAILED, SchedulerErrorReason.TIMEOUT);
    }

    static TaskResult errorResult(SchedulerErrorReason errorType) {
  ...
  private record PhaseOutcome(
      boolean completed,
      boolean hasErrors,
      boolean hasSuccessfulCampuses,
      CampusRunSummary summary) {
