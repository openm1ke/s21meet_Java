...
public class StudentCredentialsScheduler {
  ...
  public void runNightlySync() {
    if (pageSize <= 0 || batchSize <= 0) {
      log.warn("credentials scheduler пропущен: page-size={}, batch-size={}", pageSize, batchSize);
      return;
    }
    ...
    while (hasNextPage) {
      ...
      if (page.isEmpty()) {
        ...
        if (!uniquePageLogins.isEmpty()) {
          Set<String> existing =
              new HashSet<>(studentCredentialsRepository.findExistingLogins(uniquePageLogins));
          alreadySaved += existing.size();
          List<String> missingLogins =
              uniquePageLogins.stream().filter(login -> !existing.contains(login)).toList();
  ...
  private BatchCounters processLoginsInParallel(List<String> logins, String phaseTag) {
    ...
    for (int i = 0; i < logins.size(); i += batchSize) {
      ...
      for (String login : batch) {
        futures.add(
            CompletableFuture.supplyAsync(
                () -> refreshCredentials(login), credentialsSchedulerExecutor));
      }
      ...
      schedulerMetricsService.recordCredentialsSyncBatch(
          SCHEDULER_NAME, batch.size(), batchSuccess, batchNoData, batchFailed, batchElapsed);
      log.info(
          renderBatchSummary(
              phaseTag,
              batchNumber,
