...
@ConditionalOnProperty(
    name = {"projects.scheduler.enabled", "profile.service.enabled"},
    havingValue = "true")
public class StudentProjectsScheduler {
  private static final String GRAPHQL_PROVIDER = "graphql";
  ...
  public void refreshActiveProjects() {
    ...
    for (String schoolId : campuses) {
      if (remainingRunBudget <= 0) {
        log.info("projects scheduler run budget exhausted: max_logins_per_run={}", maxLoginsPerRun);
        break;
      }
    ...
    int runBudgetUsed =
        maxLoginsPerRun <= 0 ? totals.scanned() : Math.clamp(totals.scanned(), 0, maxLoginsPerRun);
    int runBudgetRemaining =
        maxLoginsPerRun <= 0 ? Integer.MAX_VALUE : Math.max(maxLoginsPerRun - runBudgetUsed, 0);
    double successRate = percent(totals.success(), processedTotal);
    double throughputPerSecond =
        elapsed.toMillis() <= 0 ? 0.0 : (processedTotal * 1000.0) / elapsed.toMillis();
    log.info(
  ...
  private CampusRunResult processCampus(
      String schoolId, OffsetDateTime staleBefore, int initialRunBudget) {
    ...
    long campusActiveTotal =
        studentCredentialsRepository.countActiveCredentialsBySchoolId(schoolId);
    long campusStaleTotalInitial =
        studentCredentialsRepository.countStaleActiveCredentialsBySchoolId(schoolId, staleBefore);
    ...
    while (remainingRunBudget > 0 && hasMorePages) {
      ...
      List<StudentCredentials> staleCredentials =
          studentCredentialsRepository.findStaleActiveCredentialsAfterBySchoolId(
              cursor, schoolId, staleBefore, PageRequest.of(0, fetchPageSize));
      ...
      logCampusProgress(
          campusName, pageNumber, campusStaleTotalInitial, counters, remainingRunBudget, cursor);
    }
  ...
  @SuppressFBWarnings(
      ...
      justification = "Multiline operational logs intentionally use fixed newline separators")
  private String buildBatchLogBlock(
      BatchLogContext context, BatchCounters counters, long elapsedMs) {
    double batchProgress = percent(context.batchNo(), context.totalBatches());
    long completed =
        (long) counters.success()
            + counters.skippedNoProvider()
            + counters.failed()
            + counters.timedOut();
    double completionRate = percent(completed, context.batchSize());
    double avgMsPerLogin =
        context.batchSize() <= 0 ? 0.0 : elapsedMs / (double) context.batchSize();
    double pageDonePercent =
        percent(context.pageProgress().doneInPage(), context.pageProgress().takenInPage());
  ...
  private BatchCounters processPage(
      List<StudentCredentials> credentials, int pageNumber, int currentBatchSize) {
    ...
    for (int i = 0; i < credentials.size(); i += effectiveBatchSize) {
      ...
      BatchExecutionResult batchExecution =
          processBatchWithRetries(batch, pageNumber, batchNo, maxAttempts);
      totals = totals.plus(batchExecution.counters());
      BatchCounters batchCounters = batchExecution.counters();
      int batchDone =
          (int)
              ((long) batchCounters.success()
  ...
  private BatchExecutionResult processBatchWithRetries(
      List<StudentCredentials> batch, int pageNumber, int batchNo, int maxAttempts) {
    ...
    for (int attempt = 1; attempt <= maxAttempts && !attemptCredentials.isEmpty(); attempt++) {
      AttemptExecutionResult attemptResult =
          executeAttempt(attemptCredentials, pageNumber, batchNo, attempt, maxAttempts);
      totals = totals.plus(attemptResult.counters());
      attemptCredentials = attemptResult.nextAttempt();
      if (!delayBeforeNextAttempt(
          pageNumber, batchNo, attemptCredentials.size(), attempt, maxAttempts)) {
        break;
      }
    }
  ...
  private AttemptExecutionResult executeAttempt(
    ...
    for (int idx = 0; idx < futures.size(); idx++) {
      ...
      RefreshResult result =
          getTaskResult(studentCredentials, future, pageNumber, batchNo, attempt);
  ...
  private boolean delayBeforeNextAttempt(
      int pageNumber, int batchNo, int pendingRetries, int attempt, int maxAttempts) {
  ...
  private void logBatchProgress(BatchLogContext context, BatchExecutionResult execution) {
    ...
    if (context.batchNo() == 1
        || context.batchNo() == context.totalBatches()
        || context.batchNo() % 10 == 0) {
      log.info("{}", logBlock);
      return;
    }
    log.debug("{}", logBlock);
  ...
  private int batchSizeForProvider(String providerType) {
    if (GRAPHQL_PROVIDER.equalsIgnoreCase(providerType)) {
  ...
  private record AttemptExecutionResult(
      BatchCounters counters, List<StudentCredentials> nextAttempt) {}

  private record BatchExecutionResult(BatchCounters counters, long elapsedMs) {}

  private record SyncCounters(
      int scanned, int success, int skippedNoProvider, int failed, int timedOut) {
  ...
  private record BatchLogContext(
      int pageNumber, int batchNo, int totalBatches, int batchSize, PageProgress pageProgress) {}
}
