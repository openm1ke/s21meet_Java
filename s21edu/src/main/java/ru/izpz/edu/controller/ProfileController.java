...
public class ProfileController {
  ...
  public ResponseEntity<ProfileDto> getProfile(
      @RequestParam("telegramId")
          ...
          @Pattern(
              regexp = "^\\d{5,13}$",
              message =
                  "Telegram ID должен содержать только цифры и быть длиной от 5 до 13 символов")
          String telegramId) {
  ...
  @GetMapping("/login")
  ResponseEntity<ParticipantDto> checkEduLogin(@RequestParam @NotBlank String login) {
  ...
  ResponseEntity<ProfileDto> checkAndSetLogin(@Valid @RequestBody ProfileRequest request) {
    log.info(
        "Получен запрос на привязку логина: login = {} для телеграма = {}",
        request.getS21login(),
  ...
  @PostMapping("/code")
  ResponseEntity<ProfileCodeResponse> sendVerificationCode(
      @Valid @RequestBody ProfileCodeRequest request) {
  ...
  ResponseEntity<ParticipantDto> getParticipant(@Valid @RequestBody ParticipantRequest request) {
    log.info(
        "Получен запрос на вывод данных участника {} для {}",
        request.getEduLogin(),
        request.getTelegramId());
  ...
  ResponseEntity<ProfileDto> setLastCommand(@Valid @RequestBody LastCommandRequest request) {
    log.info(
        "Получен запрос на обновление последней команды для telegramId = {}, команда {}",
        request.getTelegramId(),
        request.getCommand());
  ...
  ResponseEntity<FriendDto> addFriend(@Valid @RequestBody FriendRequest friendRequest) {
    ...
    log.info(
        "Получен запрос на {} у {} для {}",
        friendRequest.getAction(),
        friendRequest.getLogin(),
        profile.s21login());
    var dto =
        friendsService.applyFriend(
            profile.telegramId(),
            friendRequest.getLogin(),
            friendRequest.getAction(),
  ...
  ResponseEntity<FriendsSliceDto> getFriends(
    ...
    log.info(
        "Получен запрос на вывод друзей для telegramId = {}, page = {}, size = {}",
        telegramId,
        page,
        size);
  ...
  ResponseEntity<EventsSliceDto> getEvents(
      ...
      @RequestParam @Min(1) int size) {
    log.info(
        "Получен запрос на вывод списка событий кампуса для telegramId = {}, page = {}, size = {}",
        telegramId,
        page,
        size);
  ...
  ResponseEntity<List<ProjectExecutorDto>> getProjectExecutors(
