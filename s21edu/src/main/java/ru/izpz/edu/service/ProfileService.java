...
public class ProfileService {
        ...
        .orElseGet(
            () -> {
              ...
              try {
                ...
                log.warn(
                    "Race condition in getOrCreateProfile for telegramId={}: {}",
                    telegramId,
                    e.getMessage());
                return profileRepository
                    .findByTelegramId(telegramId)
  ...
  public ProfileDto updateProfileStatus(ProfileRequest request) {
    return profileRepository
        ...
        .map(profileMapper::toDto)
        .orElseThrow(
            () -> new EntityNotFoundException(PROFILE_NOT_FOUND_MESSAGE + request.getTelegramId()));
  }
  ...
  public ProfileDto checkAndSetLogin(String telegramId, String s21login) {
    ...
    try {
      ...
      log.warn(
          "Race condition in checkAndSetLogin for telegramId={}, s21login={}: {}",
          telegramId,
          s21login,
          e.getMessage());
            ...
            .orElseGet(
                () -> {
                  ...
                  try {
                    ...
                    log.warn(
                        "Race condition while creating verification code for s21login={}: {}",
                        s21login,
                        e.getMessage());
                    return profileValidationRepository
                        .findByS21login(s21login)
  ...
  public ParticipantDto getParticipant(String eduLogin) {
            ...
            .orElseThrow(
                () ->
                    new EntityNotFoundException(
                        PROFILE_NOT_FOUND_MESSAGE + request.getTelegramId()));
  ...
  public CampusDto getCampus(String telegramId) {
    ...
    if (s21login == null || s21login.isBlank()) {
      log.warn(
          "Не найден s21login для telegramId = {}, используем кампус по умолчанию {}",
          telegramId,
          DEFAULT_CAMPUS_NAME);
    ...
    try {
      ...
      if (participant.getCampus() != null) {
        return toCampusDto(
            participant.getCampus().getCampusName(), participant.getCampus().getId());
      }
      log.warn(
          "Не удалось определить кампус пользователя {}, используем кампус по умолчанию {}",
          s21login,
          DEFAULT_CAMPUS_NAME);
      return defaultCampus();
    } catch (PlatformClientException | IllegalStateException e) {
      log.warn(
          "Ошибка получения кампуса для {}, используем кампус по умолчанию {}",
          s21login,
          DEFAULT_CAMPUS_NAME,
          e);
      return defaultCampus();
  ...
  private void enrichPresence(ParticipantDto dto, String login) {
    ...
    if (seat != null) {
      ...
      if (seat.getId() != null) {
        ...
        if (clusterId != null) {
          clusterRepository
              .findById(clusterId)
              .ifPresent(cluster -> seatDto.setClusterName(cluster.getName()));
        }
      }
      dto.setSeat(seatDto);
    ...
    onlineRepository
        .findByLogin(login)
        .ifPresent(online -> dto.setLastSeenAt(online.getLastSeenAt()));
  }
}
