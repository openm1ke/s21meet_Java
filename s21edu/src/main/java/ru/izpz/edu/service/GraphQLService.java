...
public class GraphQLService {
  ...
  private static final String COALITION_REFRESH_DURATION =
      "edu_graphql_coalition_refresh_duration_seconds";
  private static final String PROJECTS_REFRESH_COUNTER = "edu_graphql_projects_refresh_total";
  private static final String PROJECTS_REFRESH_DURATION =
        ...
        .map(
            p ->
                new ClusterSeat(
                    ...
                    (p.experience() != null
                            && p.experience().level() != null
                            && p.experience().level().range() != null)
                        ? p.experience().level().range().levelCode()
                        : null,
                    p.stageGroupName(),
                    p.stageName()))
        .toList();
  ...
  public List<StudentProjectData> getStudentProjectsByLogin(String login) {
    ...
    GraphQLStudentProjectsDataDto data =
        client.execute(
  ...
  public List<StudentProjectData> getCachedStudentProjectsByLogin(String login) {
    ...
    return studentProjectRepository
        .findAllByLoginAndSnapshotFalseOrderBySortOrderAsc(login)
        .stream()
        .map(this::toDto)
        .toList();
  }
  ...
  public void refreshStudentProjectsByLogin(String login) {
    ...
    try {
      ...
      sample.stop(
          Timer.builder(PROJECTS_REFRESH_DURATION)
  ...
  public void refreshStudentCoalitionByLogin(String login) {
    ...
    try {
      ...
      GraphQLStudentCoalitionDataDto data =
          client.execute(
      ...
      sample.stop(
          Timer.builder(COALITION_REFRESH_DURATION)
  ...
  private boolean isCoalitionRefreshRequired(String login) {
    return studentCoalitionRepository
        .findById(login)
  ...
  private GraphQLStudentCredentialsDto fetchAndStoreCredentials(String login) {
    GraphQLStudentCredentialsDataDto data =
        client.execute(
