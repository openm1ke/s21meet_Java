...
public class CampusPersistenceService {
  ...
  public void replaceParticipantsByCampusId(String campusId, List<Workplace> workplaces) {
    Set<Long> clusterIds =
        clusterRepository.findAllByCampusIdOrderByFloorAsc(campusId).stream()
  ...
  @Transactional
  public void replaceCampusSnapshot(
      String campusId, List<Cluster> clusters, List<Workplace> workplaces) {
    Set<Long> existingClusterIds =
        clusterRepository.findAllByCampusIdOrderByFloorAsc(campusId).stream()
            .map(Cluster::getClusterId)
            .collect(toSet());
