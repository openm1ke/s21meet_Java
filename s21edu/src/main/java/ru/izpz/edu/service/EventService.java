...
public class EventService {
  ...
  public SaveEventsStats saveEvents(List<EventV1DTO> eventsDto) {
    ...
    var ids = eventsDto.stream().map(EventV1DTO::getId).toList();
    ...
    log.info(
        "saveEvents: обработано {} событий (новых: {}, обновлено: {})",
        toSave.size(),
        createdCount,
        updatedCount);
  ...
  public EventsSliceDto getEvents(int page, int size) {
    ...
    Slice<Event> eventSlice =
        eventRepository.findAllOrderedByStartDateTime(weekStart, weekEnd, pageable);

    List<EventDto> eventDtos =
        eventSlice.getContent().stream().map(eventMapper::fromEventToDto).toList();

    return new EventsSliceDto(eventDtos, page, size, eventSlice.hasNext());
  }
}
