...
public class FriendService {
  ...
  public FriendDto applyFriend(
      ...
      FriendRequest.@NotNull Action action,
      String name) {
    Friends f =
        friendsRepository
            .findFirstByTelegramIdAndLogin(telegramId, login)
            .orElseGet(
                () -> {
                  var nf = new Friends();
  ...
  public FriendsSliceDto getFriends(String telegramId, int page, int size) {
    ...
    List<String> friendLogins = slice.getContent().stream().map(Friends::getLogin).toList();

    Map<String, Workplace> occupied =
        workplaceService.findAllByLoginIn(friendLogins).stream()
    ...
    Map<Long, String> clusterNameById =
        clusterRepository.findAllByClusterIdIn(clusterIds).stream()
            .collect(Collectors.toMap(Cluster::getClusterId, Cluster::getName));

    Map<String, ParticipantStatusEnum> statusByLogin =
        participantRepository.findAllViewByLoginIn(friendLogins).stream()
            .collect(
                Collectors.toMap(
                    ParticipantView::getLogin, ParticipantView::getStatus, (a, b) -> a));

    List<FriendDto> items =
