...
public class RestApiProjectsProvider implements ProjectsProvider {
  ...
  private FetchResult fetchAllStatuses(String login) {
    ...
    try {
      ParticipantProjectsV1DTO response =
          restProjectsApiFacade.getParticipantProjectsByLogin(login, pageSize, null);
      if (response == null) {
  ...
  private void appendDistinct(
      List<StudentProjectData> target, Set<Long> seenIds, List<ParticipantProjectV1DTO> projects) {
    for (ParticipantProjectV1DTO project : projects) {
      if (project != null && !isAssigned(project)) {
