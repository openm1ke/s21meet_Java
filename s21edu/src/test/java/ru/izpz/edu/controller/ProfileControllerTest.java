...
class ProfileControllerTest {
  ...
  @MockitoBean private CampusService campusService;

  @MockitoBean private FriendService friendsService;

  @MockitoBean private EventService eventService;
  @MockitoBean private ProjectDirectoryService projectDirectoryService;

  @MockitoBean private CampusClient campusClient;

  @MockitoBean private NotifyService notifyService;

  @MockitoBean private GraphQLService graphQlService;

  @MockitoBean private PlatformApiFacade platformApiFacade;
  @MockitoBean private CampusRoutingProjectsProvider campusRoutingProjectsProvider;
  ...
  void getProfile_shouldReturnOk() throws Exception {
    ...
    mockMvc
        .perform(get("/profile").param("telegramId", "123456"))
  ...
  void checkAndSetLogin_shouldReturnOk() throws Exception {
    ProfileRequest req = ProfileRequest.builder().telegramId("123456").s21login("newlogin").build();
  ...
  void checkEduLogin_shouldReturnOk() throws Exception {
    ...
    mockMvc
        .perform(get("/profile/login").param("login", "login"))
        .andExpect(status().isOk())
        .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON));
  ...
  void getCampus_shouldReturnOk() throws Exception {
    ...
    CampusService.CampusSnapshot snapshot =
        new CampusService.CampusSnapshot(List.of(), Map.of("No data", 1L));
  ...
  void getParticipant_shouldReturnOk() throws Exception {
    ParticipantRequest req =
        ParticipantRequest.builder().telegramId("123456").eduLogin("edu").build();
  ...
  void setLastCommand_shouldReturnOk() throws Exception {
    ...
    LastCommandRequest req =
        LastCommandRequest.builder().telegramId("123456").command(state).build();
  ...
  void getFriends_shouldReturnOk() throws Exception {
    FriendsSliceDto slice =
        new FriendsSliceDto(
            List.of(FriendDto.builder().login("u").telegramId("123456").build()), 0, 10, false);
  ...
  void addFriend_shouldReturnOk() throws Exception {
    ...
    when(profileService.getProfile("123456"))
        .thenReturn(new ProfileDto("123456", "me", ProfileStatus.CREATED, null));
    when(friendsService.applyFriend(
            eq("123456"), eq("friend"), eq(FriendRequest.Action.TOGGLE_FRIEND), any()))
        .thenReturn(
            FriendDto.builder().login("friend").telegramId("123456").isFriend(true).build());
    ...
    verify(friendsService)
        .applyFriend(eq("123456"), eq("friend"), eq(FriendRequest.Action.TOGGLE_FRIEND), any());
  }
  ...
  void getEvent_shouldReturnOk() throws Exception {
    ...
    mockMvc
        .perform(get("/profile/event").param("id", "1"))
        .andExpect(status().isOk())
  ...
  void getProjects_shouldReturnOk() throws Exception {
    ...
    mockMvc
        .perform(get("/profile/projects").param("login", "login"))
        .andExpect(status().isOk())
  ...
  void getProjectNames_shouldReturnOk() throws Exception {
    ...
    when(profileService.getProjectNamesByTelegramId("123456"))
        .thenReturn(List.of("C2_SimpleBashUtils", "C3_s21_stringplus"));

    mockMvc
        .perform(
            post("/profile/project-names")
                .contentType(MediaType.APPLICATION_JSON)
  ...
  void getProjectExecutors_shouldReturnOk() throws Exception {
    ...
    mockMvc
        .perform(
            post("/profile/project-executors")
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(request)))
        .andExpect(status().isOk())
  ...
  void getAllProjectNames_shouldReturnOk() throws Exception {
    when(projectDirectoryService.getProjectNames())
        .thenReturn(List.of("A1_Maze_C", "C2_SimpleBashUtils"));

    mockMvc
        .perform(post("/profile/project-names/all"))
        .andExpect(status().isOk())
  ...
  void getProjectExecutors_shouldReturnBadRequest_whenProjectNameBlank() throws Exception {
    ...
    verify(projectDirectoryService, never())
        .getProjectExecutors(any(ProjectExecutorsRequest.class));
  }
