...
class SchedulerMetricsAspectTest {
  ...
  @org.springframework.beans.factory.annotation.Autowired private TestScheduler testScheduler;
  ...
  void shouldRecordSuccessMetrics() {
    ...
    verify(schedulerMetricsService)
        .recordPhaseRequest("test_scheduler", "test_phase", SchedulerPhaseRequestStatus.SUCCESS);
    ...
    verify(schedulerMetricsService, never())
        .recordPhaseIssue(eq("test_scheduler"), eq("test_phase"), any());
  }
  ...
  void shouldRecordFailureMetrics() {
    ...
    verify(schedulerMetricsService).startPhaseTimer();
    verify(schedulerMetricsService)
        .recordPhaseRequest("test_scheduler", "test_phase", SchedulerPhaseRequestStatus.FAILED);
    verify(schedulerMetricsService)
        .recordPhaseIssue("test_scheduler", "test_phase", SchedulerErrorReason.EXECUTION_EXCEPTION);
