...
class CampusSchedulerTest {
    ...
    scheduler =
        new CampusScheduler(
            ...
            new SchedulerMetricsService(
                meterRegistry, campusCatalog, new SchedulerErrorClassifier()),
            campusCatalog,
            schedulerProperties,
            executorService);
  }
  ...
  void parseMskKznNsk_success_updatesClustersAndParticipants() {
    ...
    when(campusService.fetchParticipantsByClusterWithProvider(2L))
        .thenReturn(List.of(new Workplace()));
    ...
    verify(campusService, times(3))
        .replaceCampusSnapshotByCampusId(
            campusIdCaptor.capture(),
            clustersCaptor.capture(),
            ArgumentMatchers.<Workplace>anyList());
    ...
    verify(campusService)
        .replaceCampusSnapshotByCampusId(eq(MSK), anyList(), argThat(list -> list.size() == 1));
    verify(campusService)
        .replaceCampusSnapshotByCampusId(eq(KZN), anyList(), argThat(list -> list.size() == 1));
    verify(campusService)
        .replaceCampusSnapshotByCampusId(eq(NSK), anyList(), argThat(list -> list.size() == 1));
  ...
  void parseMskKznNsk_whenApiExceptions_doesNotThrowAndContinues() {
    ...
    when(campusClient.getClustersByCampus(MSK)).thenReturn(List.of(c1));
    when(campusClient.getClustersByCampus(KZN))
    ...
    verify(campusService).fetchParticipantsByClusterWithProvider(3L);
    verify(campusService, times(2))
        .replaceCampusSnapshotByCampusId(anyString(), anyList(), anyList());
