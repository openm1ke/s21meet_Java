...
class CampusSchedulerFailurePathsTest {
  ...
  void parseMskKznNsk_perCampusTimeoutAfterPartialSuccess_marksRunAsPartial() {
    ...
    when(campusService.fetchParticipantsByClusterWithProvider(mskCluster.getId()))
    ...
    doAnswer(
        ...
        .when(campusService)
        .fetchParticipantsByClusterWithProvider(kznCluster.getId());
    doAnswer(
            invocation -> {
              blockUntilInterrupted();
              return List.<Workplace>of();
    ...
    verify(campusService, atLeastOnce())
        .replaceCampusSnapshotByCampusId(anyString(), anyList(), anyList());
  }
        ...
        .thenAnswer(
            invocation -> {
              if (callOrdinal.getAndIncrement() == 0) {
                throw new PlatformClientException("boom", null);
              }
              ClusterV1DTO cluster = new ClusterV1DTO();
              cluster.setId(1L);
              cluster.setName("cluster");
  ...
  void parseMskKznNsk_participantsError_recordsPartialStatus() {
    ...
    cluster.setName("cluster");
    when(campusClient.getClustersByCampus(anyString())).thenReturn(List.of(cluster));
    doThrow(new PlatformClientException("boom", null))
        .when(campusService)
        .fetchParticipantsByClusterWithProvider(cluster.getId());
