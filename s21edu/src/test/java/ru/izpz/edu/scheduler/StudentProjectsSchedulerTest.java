...
class StudentProjectsSchedulerTest {
  ...
  void refreshActiveProjects_prioritizesCampusesAndProcessesOnlyStale() {
    ...
    when(campusRoutingProjectsProvider.providerTypeForSchoolId())
        .thenReturn("graphql", "rest", "rest");
    ...
    when(studentCredentialsRepository.countStaleActiveCredentialsBySchoolId(
            eq(mskId), any(OffsetDateTime.class)))
        .thenReturn(1L);
    when(studentCredentialsRepository.countStaleActiveCredentialsBySchoolId(
            eq(kznId), any(OffsetDateTime.class)))
        .thenReturn(1L);
    when(studentCredentialsRepository.countStaleActiveCredentialsBySchoolId(
            eq(nskId), any(OffsetDateTime.class)))
        .thenReturn(0L);
    when(studentCredentialsRepository.findStaleActiveCredentialsAfterBySchoolId(
            eq(""), eq(mskId), any(OffsetDateTime.class), any(Pageable.class)))
        .thenReturn(List.of(msk));
    when(studentCredentialsRepository.findStaleActiveCredentialsAfterBySchoolId(
            eq("a1"), eq(mskId), any(OffsetDateTime.class), any(Pageable.class)))
        .thenReturn(List.of());
    when(studentCredentialsRepository.findStaleActiveCredentialsAfterBySchoolId(
            eq(""), eq(kznId), any(OffsetDateTime.class), any(Pageable.class)))
        .thenReturn(List.of(kzn));
    when(studentCredentialsRepository.findStaleActiveCredentialsAfterBySchoolId(
            eq("a2"), eq(kznId), any(OffsetDateTime.class), any(Pageable.class)))
        .thenReturn(List.of());
    when(studentCredentialsRepository.findStaleActiveCredentialsAfterBySchoolId(
            eq(""), eq(nskId), any(OffsetDateTime.class), any(Pageable.class)))
        .thenReturn(List.of());
  ...
  void refreshActiveProjects_skipsWhenInvalidConfig() {
    ...
    verify(studentCredentialsRepository, never())
        .findStaleActiveCredentialsAfterBySchoolId(any(), any(), any(), any(Pageable.class));
    verify(studentCredentialsRepository, never()).countActiveCredentialsBySchoolId(any());
    verify(campusRoutingProjectsProvider, never())
        .refreshStudentProjects(any(StudentCredentials.class));
  }
  ...
  void refreshActiveProjects_handlesSkippedAndFailedResults() {
    ...
    when(studentCredentialsRepository.findStaleActiveCredentialsAfterBySchoolId(
            eq("fail-login"), eq(mskId), any(OffsetDateTime.class), any(Pageable.class)))
        .thenReturn(List.of());
  ...
  void refreshActiveProjects_coversMiddleBatchDebugPath() {
    String mskId = "6bfe3c56-0211-4fe1-9e59-51616caac4dd";
    final StudentCredentials c1 = credential("a1", "u1", mskId);
    final StudentCredentials c2 = credential("a2", "u2", mskId);
    final StudentCredentials c3 = credential("a3", "u3", mskId);
    ...
    when(studentCredentialsRepository.findStaleActiveCredentialsAfterBySchoolId(
            eq("a3"), eq(mskId), any(OffsetDateTime.class), any(Pageable.class)))
        .thenReturn(List.of());
  ...
  void refreshActiveProjects_shouldRespectMaxLoginsPerRunBudget() {
    String mskId = "6bfe3c56-0211-4fe1-9e59-51616caac4dd";
    final StudentCredentials first = credential("a1", "u1", mskId);
    final StudentCredentials second = credential("a2", "u2", mskId);
