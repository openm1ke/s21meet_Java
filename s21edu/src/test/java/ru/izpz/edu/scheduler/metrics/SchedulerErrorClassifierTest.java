...
class SchedulerErrorClassifierTest {
  ...
  void classify_shouldReturnApiException_whenApiException() {
    assertEquals(
        SchedulerErrorReason.API_EXCEPTION,
        classifier.classify(new PlatformClientException("api", null)));
  }

  @Test
  void classify_shouldReturnInterrupted_whenInterruptedException() {
    assertEquals(
        SchedulerErrorReason.INTERRUPTED, classifier.classify(new InterruptedException("stop")));
  }

  @Test
  void classify_shouldReturnTimeout_whenContainsTimeoutKeywords() {
    assertEquals(
        SchedulerErrorReason.TIMEOUT,
        classifier.classify(new RuntimeException("operation timed out")));
  }

  @Test
  void classify_shouldReturnInterrupted_whenContainsInterruptedKeyword() {
    assertEquals(
        SchedulerErrorReason.INTERRUPTED,
        classifier.classify(new RuntimeException("thread interrupted")));
  }

  @Test
  void classify_shouldReturnNetwork_whenContainsNetworkKeywords() {
    assertEquals(
        SchedulerErrorReason.NETWORK,
        classifier.classify(new RuntimeException("connection refused")));
  }

  @Test
  void classify_shouldReturnRateLimit_whenContainsRateKeywords() {
    assertEquals(
        SchedulerErrorReason.RATE_LIMIT,
        classifier.classify(new RuntimeException("too many requests 429")));
  }

  @Test
  void classify_shouldReturnExecutionException_whenNoKnownKeywords() {
    assertEquals(
        SchedulerErrorReason.EXECUTION_EXCEPTION,
        classifier.classify(new RuntimeException("boom")));
  }
