...
class StudentProjectRefreshServiceTest {
  ...
  void replaceProjects_shouldStoreSnapshot_whenProjectsEmpty() {
    StudentProjectRefreshService service =
        new StudentProjectRefreshService(studentProjectRepository);
  ...
  void replaceProjects_shouldStoreMappedProjects_whenProjectsExist() {
    ...
    verify(studentProjectRepository)
        .saveAll(
            argThat(
                iterable -> {
                  List<StudentProject> list =
                      (iterable instanceof List<StudentProject> l) ? l : List.of();
                  return list.size() == 2
