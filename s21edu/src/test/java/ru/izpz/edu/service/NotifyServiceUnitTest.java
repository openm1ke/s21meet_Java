...
class NotifyServiceUnitTest {
  ...
  void computeAndPersistChanges_returnsEmpty_whenNoTrackedLogins() {
    ...
    final WorkplaceRepository workplaceRepository = mock(WorkplaceRepository.class);
    final OnlineRepository onlineRepository = mock(OnlineRepository.class);
    final NotifyService service =
        new NotifyService(friendsRepository, workplaceRepository, onlineRepository);
  ...
  void computeAndPersistChanges_doesNotEmitChange_whenNoSubscribers() {
    final FriendsRepository friendsRepository = mock(FriendsRepository.class);
    final WorkplaceRepository workplaceRepository = mock(WorkplaceRepository.class);
    final OnlineRepository onlineRepository = mock(OnlineRepository.class);
    final NotifyService service =
        new NotifyService(friendsRepository, workplaceRepository, onlineRepository);
